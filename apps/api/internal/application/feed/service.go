...
func (s *Service) GetFeed(ctx context.Context, req FeedRequest) (*FeedResult, error) {
	...
	strategy, ok := s.router.Route(req.Scene)
	if !ok {
...
func (s *TimelineStrategy) List(ctx context.Context, req FeedRequest) (*FeedResult, error) {
	_, err := parseTimelineCursor(req.Cursor)
	if err != nil {
		return nil, err
	}
...
func (s *HotStrategy) List(ctx context.Context, req FeedRequest) (*FeedResult, error) {
	...
	_ = parsedCursor // cursor validation is retained here; source owns candidate loading.
	page, err := NewHotCandidateSource(s.repo, s.cache).Load(ctx, req, limit)
	if err != nil {
		return nil, err
	}
...
func (s *FollowingStrategy) List(ctx context.Context, req FeedRequest) (*FeedResult, error) {
	...
	page, err := NewFollowingCandidateSource(s.repo, s.followingIndex).Load(ctx, req, limit)
	if err != nil {
		return nil, err
	}
