...
**状态：已完成。** 第一批已在推荐服务内部建立
`Multi-Source Recall -> Dedup -> Pre-Rank Top200 -> Rank Top50 -> Author Re-Rank`
主链路：热门、最新、关注和兴趣候选并行各取 Top100，预排使用热度、时效与关注
信号，精排复用用户兴趣向量和视频内容向量。第二批已接入 Similar Video Recall
和 Collaborative Recall：前者基于最近一次正向观看与内容向量召回，后者基于
30 天正向观看共现召回；六路召回均已补齐候选数量和耗时指标。第三批将播放时长、
完播、点赞、收藏、评论和关注作者内容按权重汇总为用户兴趣向量，相关状态变化会
事务内失效旧画像。内容侧组合内容向量、互动热度、时效、作者关系和召回来源特征；
曝光过滤继续复用 Stage 4 的 Seen Bloom。
...
**状态：已完成。** 第一批已在 API 入口增加按客户端 IP 的有界 Token Bucket 限流和
请求 Context 超时，并为 HTTP Server 配置读写超时与 SIGINT／SIGTERM 优雅关闭。
推荐主召回使用独立 500 ms 超时，失败后依次降级到 Hot、Latest；降级页不透传不兼容的
推荐游标。上传链路保留已有两分钟媒体处理超时，不套用三秒普通 API 截止时间。第二批为
推荐主召回增加 32 并发隔舱、连续 5 次失败熔断 10 秒及同一总超时内的一次瞬时错误重试；
熔断恢复仅放行单个探测请求，参数错误不重试、不降级。第三批完成局部召回失败开放、画像
异常使用默认画像、向量异常回退热度／时效排序，并增加治理决策指标和受 Internal Token
保护的推荐手动降级开关；开关为单进程临时状态，重启自动复位。
